regression_plane <- function(model, n_points = 100, mesh=FALSE, mesh_step=1) {
  ...
  p <- plot_ly(x = data[[terms[1]]],
               y = data[[terms[2]]],
               z = data[[outcome_name]],
               opacity = 0.6
               ) %>%
    add_markers(marker = list(size=3), showlegend=FALSE, hoverinfo='none') %>%
  ...
  z_hat <- predict(model, newdata = predictor.grid)
  ...
  if (mesh) {
    ...
    for (i in 1:ncol(z_hat2)) {
      p <- p %>% add_trace(type="scatter3d", x=x2[i], y=y2, z=z_hat2[,i],
